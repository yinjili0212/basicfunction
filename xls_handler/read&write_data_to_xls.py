...
print(sheet2)
## 获取工作表格的3种方法，第3种方法
sheet3 = workbook.sheet_by_name("2019年")   #workbook.sheet_by_name(sheet名称)：此前提是知道表格中的sheet名称
print(sheet3)
# 获取工作表格的所有sheet名称
...
data = sheet1.cell(rowx = 2, colx = 2)  # 获取第 3 行，第 3 列对应的单元格数据
...
#
# workbook2 = copy(wb = workbook)
# nrows = workbook.sheet_by_name('2019年').nrows
# ncols = workbook.sheet_by_name('2019年').ncols
# worksheet = workbook2.get_sheet('2019年')
# worksheet.write(0,0,'1') #第一个参数=（行数-1），第2个参数=（列数-1），第三个参数=需要写入的数据
# workbook2.save(r'./编程语言排名.xls')
